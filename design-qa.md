...
- 真机证据：设备当时没有 GPS 新鲜定位，系统只提供约两天前的网络/融合坐标；旧前端又会在恢复缓存时续写当前时间，导致过期坐标长期被当成当前位置。
- 同坐标核验：`38.899294, 115.476267` 经高德逆地理返回“保定市竞秀区韩村北路街道新世纪花园B区”，与页面原先写入的“莲池区当前位置”不一致。
- 修正：所有定位结果先按最终经纬度做一次逆地理校正；首页与地图显示小区/村庄等精确地点，地区搜索与“我的”显示“精确地点 · 市 · 区县”。手动地区仍保持最高优先级。
- 首次授权：用户点击“重新定位”后立即调用浏览器高精度定位，保留移动浏览器要求的用户手势；系统定位失败后才进入高德定位兜底，避免延迟请求导致小米浏览器不弹网站授权。
- 冷启动：网站定位权限为 `prompt/denied` 时不启动后台静默定位，避免占用 `isLocating` 导致“重新定位”点击被忽略；已授权的静默刷新设 12 秒解锁上限。
- 缓存恢复：version 2 坐标在执行任何 250ms 后台权限检查前同步写入运行态 `hasLocation/userLat/userLng`；有效缓存不会再因集市数据尚未加载而被误判清除。
- 入口收口：定位失败卡的“重新定位”只调用显式定位，不再走静默刷新；定位过程保持在新版首页/地图当前页，不再跳入已废弃的旧“附近页”并显示旧全量列表。
- 缓存：定位缓存升级为 version 2，最长有效 30 分钟；旧版、超时或缺少时间戳的 GPS 状态在首屏同步失效，不再等待集市数据加载，也不再因缓存恢复而续期；由该失效 GPS 写入且未被手动选择的最近地区同步移除。
- 浏览器回归：旧“莲池区”状态与旧坐标缓存重载后均被清空；模拟校正结果后地图显示“新世纪花园B区”，地区搜索与“我的”显示“新世纪花园B区 · 保定市 · 竞秀区”。
- 自动化：内联脚本语法通过，全量 pytest `59 passed`。
